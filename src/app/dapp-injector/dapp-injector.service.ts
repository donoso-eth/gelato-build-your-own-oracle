...
import BuildOracleMetadata from 'src/assets/contracts/build-oracle_metadata.json';
...
export class DappInjector implements OnDestroy {
  ...
  DAPP_STATE:IDAPP_STATE<BuildOracle> = {
...
async localWallet(index:number) {
 ...
 private  async walletInitialization() {
  ...
  private async contractInitialization() {
    ...
    const contract = new AngularContract<BuildOracle>({
      metadata:  BuildOracleMetadata,
